...
const { btnGroup, dataObj, registerCellInfo } = Register;
...
describe("Registry", () => {
  it("initializes data Obj holder with first register", () => {
    ...
    expect(dataObj).toMatchObject({ [NAME_REF_1]: INIT_VAL_1 });
    expect(btnGroup[MUTUAL_GROUP_MANE_1].has(NAME_REF_1)).toBe(true);
  });
  it("pushes second obj with same group name", () => {
    ...
    expect(btnGroup[MUTUAL_GROUP_MANE_1].has(NAME_REF_1)).toBe(true);
    expect(btnGroup[MUTUAL_GROUP_MANE_1].has(NAME_REF_2)).toBe(true);
  });
  it("tests btnGroup with 2 branches of group name", () => {
    ...
    expect(btnGroup[MUTUAL_GROUP_MANE_1].has(NAME_REF_1)).toBe(true);
    expect(btnGroup[MUTUAL_GROUP_MANE_1].has(NAME_REF_2)).toBe(true);
    expect(btnGroup[MUTUAL_GROUP_MANE_2].has(NAME_REF_3)).toBe(true);
  });
